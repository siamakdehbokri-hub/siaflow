-- Add saving categories to handle_new_user function
...
BEGIN
  ...
  INSERT INTO public.categories (user_id, name, icon, color, budget, type, subcategories) VALUES
    ...
    (NEW.id, 'مالی و بانک', 'Landmark', 'hsl(220, 70%, 45%)', 5000000, 'expense', 
     ARRAY['اقساط و بدهی', 'وام', 'کارمزد انتقال']),
    ...
    -- Saving Categories (NEW - Separate from expenses)
    (NEW.id, 'پس‌انداز و سرمایه‌گذاری', 'PiggyBank', 'hsl(175, 85%, 42%)', NULL, 'saving', 
     ARRAY['پس‌انداز ماهانه', 'صندوق اضطراری', 'سرمایه‌گذاری بلندمدت']),
    
    (NEW.id, 'خرید سرمایه‌ای', 'Coins', 'hsl(45, 93%, 47%)', NULL, 'saving', 
     ARRAY['طلا و سکه', 'ارز دیجیتال', 'سهام', 'ملک']),
    
    (NEW.id, 'اهداف مالی', 'Target', 'hsl(210, 80%, 55%)', NULL, 'saving', 
     ARRAY['خرید خانه', 'خرید ماشین', 'سفر', 'تحصیل']);
