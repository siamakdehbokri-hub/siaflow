  ...
  return <Sheet>
      <SheetTrigger asChild>
        <Button variant="ghost" size="icon" className="relative w-9 h-9 rounded-xl hover:bg-primary/10 hover:shadow-glow-sm transition-all duration-300 group">
          ...
          {reminders.length > 0 && <span className="absolute inset-0 rounded-xl border-2 border-destructive/30 animate-ping opacity-50" />}
        </Button>
      </SheetTrigger>
      <SheetContent side="bottom" className="h-[70vh]">
        ...
        <div className="mt-6 space-y-3 overflow-y-auto max-h-[50vh]">
          {reminders.length === 0 ? <div className="text-center py-8 text-muted-foreground">
            ...
            </div> : reminders.map(reminder => <Card key={reminder.id} variant="glass" className="relative overflow-hidden">
                {reminder.daysUntilDue === 0 && <div className="absolute inset-0 bg-destructive/5 animate-pulse" />}
                <CardContent className="p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        {reminder.daysUntilDue === 0 ? <AlertTriangle className="w-4 h-4 text-destructive" /> : <Calendar className="w-4 h-4 text-muted-foreground" />}
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button variant="ghost" size="icon-sm" onClick={() => onDismiss(reminder.id)} className="h-8 w-8">
                        <X className="w-4 h-4" />
                      </Button>
                    </div>
                  </div>
