  ...
  return <Sheet>
      <SheetTrigger asChild>
        <Button variant="ghost" size="icon" className="relative w-9 h-9 rounded-xl hover:bg-primary/10 hover:shadow-glow-sm transition-all duration-300 group">
          ...
          {overdueCount > 0 && <span className="absolute inset-0 rounded-xl border-2 border-destructive/30 animate-ping opacity-50" />}
        </Button>
      </SheetTrigger>
      <SheetContent side="bottom" className="h-[75vh]">
        ...
        {/* Stats */}
        {reminders.length > 0 && <div className="mt-4 flex gap-3">
            {overdueCount > 0 && <Badge variant="destructive" className="rounded-lg py-1.5 px-3">
                <AlertTriangle className="w-3.5 h-3.5 ml-1.5" />
                {overdueCount} عقب‌افتاده
              </Badge>}
            {todayCount > 0 && <Badge className="rounded-lg py-1.5 px-3 bg-amber-500 text-white">
        ...
        {'Notification' in window && Notification.permission === 'default' && <Button variant="outline" className="w-full mt-4 rounded-xl" onClick={onEnableNotifications}>
            <Bell className="w-4 h-4 ml-2" />
            فعال‌سازی نوتیفیکیشن مرورگر
          </Button>}

        <div className="mt-6 space-y-3 overflow-y-auto max-h-[50vh]">
          {reminders.length === 0 ? <div className="text-center py-8 text-muted-foreground">
            ...
            </div> : reminders.map(reminder => <Card key={reminder.id} className={cn("relative overflow-hidden glass", reminder.isOverdue && "border-destructive/30")}>
                {reminder.isOverdue && <div className="absolute inset-0 bg-destructive/5" />}
                {reminder.daysUntilDue === 0 && !reminder.isOverdue && <div className="absolute inset-0 bg-amber-500/5 animate-pulse" />}
                <CardContent className="p-4 relative">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-2">
                        {reminder.isOverdue ? <AlertTriangle className="w-4 h-4 text-destructive" /> : reminder.daysUntilDue === 0 ? <Clock className="w-4 h-4 text-amber-500" /> : <Calendar className="w-4 h-4 text-muted-foreground" />}
                    ...
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <Button variant="ghost" size="sm" onClick={() => onDismiss(reminder.id)} className="h-7 px-2 text-xs">
